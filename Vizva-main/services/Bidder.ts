...
export default class LazyBidder {
   ...
   * @param {ethers.BigNumber | string} tokenId the id of the un-minted NFT
   * @property {ethers.BigNumber | string} bid the amount  (in wei) that the bidder ready to pay.
   * @property {ethers.BytesLike} signature an EIP-712 signature of all fields in the NFTVoucher, apart from signature itself.
   *
   * @returns {NFTVoucher}
   */
  
  async createBidVoucher(asset: string, tokenAddress: string, tokenId: string , marketId: string, bid: ethers.BigNumberish) {
